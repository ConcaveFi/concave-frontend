    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('#midPrice', () => {
        ...
        expect(
          route.midPrice
            .invert()
            .quote(
              CurrencyAmount.fromRawAmount(route.output, decimalize(1234, route.output.decimals)),
            )
            .toExact(),
        ).toEqual(
          CurrencyAmount.fromRawAmount(route.input, decimalize(1, route.input.decimals)).toExact(),
        )
      ...
      describe('Trade', () => {
        ...
        it('TradeType.EXACT_INPUT', () => {
          ...
          expect(trade.executionPrice.invert().quote(expectedOutputAmount).quotient).toEqual(
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          ...
          expect(trade.executionPrice.toSignificant(18)).toEqual('1.66249791562447891')
          expect(trade.executionPrice.invert().toSignificant(18)).toEqual('0.601504513540621866')
          expect(trade.executionPrice.quote(expectedInputAmount).quotient).toEqual(
            outputAmount.quotient,
          )
          expect(trade.executionPrice.invert().quote(outputAmount).quotient).toEqual(
            expectedInputAmount.quotient,
          )
