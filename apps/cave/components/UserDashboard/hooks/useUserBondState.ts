...
export const useUserBondState = () => {
  ...
  return useQuery(['userBondState', address, chainId], async () => {
    ...
    const bondingContract = new Contract(
      BOND_ADDRESS[chainId],
      BondAbi,
      concaveProvider({ chainId }),
    )
    const getUserPositionsLength = (await bondingContract.getUserPositionCount(
      address,
    )) as BigNumber
    const termData: BigNumber = await bondingContract.term()
    const positions = await Promise.all(
      new Array(getUserPositionsLength.toNumber()).fill(null).map(async (_, i) => {
        const { owed, redeemed, creation }: PositionType = await bondingContract.positions(
          address,
          i,
        )
